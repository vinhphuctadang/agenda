...
module.exports = async function(jobName, definition) {
  ...
  if (s.topology.connections && s.topology.connections().length === 0 && !this._mongoUseUnifiedTopology) {
    ...
    const jobInDb = await this._collection.findOne(JOB_PROCESS_WHERE_QUERY);
    let job; // job stored in nodejs
    debug('There is a job: ' + JSON.stringify(jobInDb));

    if (jobInDb) {
      // not really bad idea on shardDB: although 'request' are sent to all shards but it's fine since we have indexing
      // secondly: minial code fix, do not change much in logic
      const result = await this._collection.findOneAndUpdate({_id: jobInDb._id}, JOB_PROCESS_SET_QUERY, JOB_RETURN_QUERY);
